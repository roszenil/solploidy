...
Most significantly, we found that the effects of ploidy, inferred as significant when it is the only trait under consideration, are overshadowed by hidden traits, or breeding system when added to models.  %R: added hidden trait.
...
Motivated by the widespread findings of polyploidy events in the ancestry of angiosperms, we also examined and found modest support for diploidization in the history of the family. %B: how about "modest"? It's 3-ish, but "positive" seems like a really idiosyncratic usage to me, despite kass & raftery. was: %B: "mixed" "not strong" what's the right word here?  E: not "weak", maybe "not strong" or "not nearly as strong as anything else" 
%R- I think modest is a good way to express it because it is the best we can do with chromosome numbers.
...
The addition of a hidden trait results in blurring of differences in diversification rates between diploids and polyploids. %B: fix fig ref 
%R- added the figure reference
...
Among SC species, diploids and polyploids are associated with similar diversification rates (\cref{figure:netdivall}E) %B: fix fig ref
% R- Figure reference is fixed
...
When we inferred only the role of breeding system in the diversification process, we were able to replicate the slower net diversification for SC  (\cref{figure:netdivall}C) .  %B: fix fig ref
...
% R- I didn't fit it without diversification but I can and I can definitely do stochastic map of both (do you want it for the revision?)
%
...
We find that, in Solanaceae, SI diploid lineages are much more likely to take a
...
% R- I commented out the rare statement above because most of the community does not agree with this statement anymore, diploidization is now considered the rule rather than the exception. Change it for the phrase below
...
Genomic evidence indicates that a polyploidy event may have occurred prior to the origin of the family Solanaceae, rendering all current `diploids' secondarily derived. 
