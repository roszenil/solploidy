...
% B: notes to self.
% Many flowering plants are thought to have experienced a WGD. (cite)
% \citet{ku_2000} and \citet{blanc2004} posited that the lineage leading to tomato, \textit{Solanum lycopersicum}, may have experienced one or more paleopolyploid events.
% A subsequent analysis of synteny between grape and \textit{Solanum} genomes, as well as the distribution between inferred paralogs within \textit{Solanum} (tomato and potato) genomes both suggest that this lineage experienced a likely round of ancient genome duplication or triplication \citep{tomato_2012}. 
%The age of this bump of paralogs was approximated using Ks distances to be around 71 million years \citep{tomato_2012}.
% If this is the case, then the genomes are re-diploidized, because low and common chromosome numbers in related families (coffee n=11, tomato n=12, etc.). 
%Not unlikely, because Arabidopsis n=5.
% On the other hand, \citet{wu_2008} find no evidence for recent polyploidization-diploidization, based on map-based synteny. 
% Simple genome re-arrangements appear sufficient to explain chromosomal evolution of x=12 (tomato-tobacco) group.
% (Add what this has to do with re-diploidization and how to interpret $\delta$.
