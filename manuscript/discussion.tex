...
\subsection*{Disentangling trait effects on diversification} %B: Emma may not like "effects,"?
...
The addition of a hidden trait results in blurring of differences in diversification rates between diploids and polyploids (Figure 2A). %B: fix fig ref
...
Among SC species, diploids and polyploids are associated with similar diversification rates (Figure 2F). %B: fix fig ref
...
When we inferred only the role of breeding system in the diversification process, we were able to replicate the slower net diversification for SC (Figure 2C).  %B: fix fig ref
...
%B: This should be to mutual liking. IMO, the support is STRONG in the apparently meager analyses, one with only D/P), which yields highly unreasonable ancestral states. But it is decidedly not strong, i.e. "positive," in the apparently more informative joint trait analyses.
We find strong support for including the diploidization rate parameter ($\delta$) in analyses that include only ploidy \cref{table:bayesfactors}. %B <- %Rosana, inspect this please. ->
...
%B: "Genomic evidence" reads as if the genomes support this. They do not--the data used in studies, with requisite flaws and problems, are derived from genomes. But I don't think that it's okay to call it "genomic evidence". It would be a bit like referring to evidence in this paper as "evolutionary evidence".
