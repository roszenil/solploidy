...
Breeding system states were scored as self-incompatible, $I$, or self-compatible, $C$, based on results curated from the literature \citep[as compiled in][]{igic_2006, goldberg_2010, robertson_2011, goldberg_2012}. %IM suggestion
...
We quantify the relative contribution of these pathways to polyploidy in two ways, each using the median estimates of rates from the simplest model that includes both traits (M16). Our results will differ from those of \citet{robertson_2011} because our transition rate estimates come from a dated phylogeny and a model that allows for state-dependent diversification. %IM suggestion
...
We can still use ratios, however, to consider the relative contribution of each pathway, analogous to the normalized age structure in a growing population \citep{leslie_1945}.
 % R- here a reference would be nice  E: Does Jake have a favorite ref? Otherwise, I guess a basic ecology textbook.-DONE
%
...
% Itay says remove but some reviewers were interested in the distribution of the states. I say leave we don't need to discuss. I'll mention briefly.
...
In the hidden-trait models, all species could take on either of two `hidden' character states. %IM: #33 %B: NOT USING A/B because that confuses panel A and panel B designations unnecessarily. 
