...
% FIXME: Rosana will update numbers around here
...
Parameters in this case are speciation and extinction rates for the hidden states ($\lambda_A,\ \lambda_B,\ \mu_A,\ \mu_B$), along with polyploidization ($\rho$) and transitions between hidden states ($\alpha$).
...
We would like to test whether including an additional trait significantly improves the model fit.
For example, does the model with both breeding system and ploidy provide a significantly better fit than the model with only ploidy?
To answer this, it is not valid to compare the $IC/CD/CP$ and $D/P$ model fits, as defined above, because they use different data.
Similarly, the $IC/CD/CP$ and $I/C$ models cannot be compared statistically.
Instead, we define lumped models \citep{tarasov_2019} that use the state space of the $IC/CD/CP$ model but constrain the rate parameters to mimic the behavior of the single-trait models. % should ref a figure

% E: We now have so many models to describe, I'm feeling like all this text may not be the best way.  I'm wondering if we can rely on figures/tables instead, and only write out the overall strategy.
...
% FIXME Rosana, please update about the parameters
