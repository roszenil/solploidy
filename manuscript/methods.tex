...
In order to investigate the association between ploidy level and diversification, we first defined a model in which taxa were classified as diploid $(D)$ or polyploid $(P)$ (model M1).
This is the same as one of the models presented in  \citet{mayrose_2011}, which was used for the exact same question across an array of plant genera.
...
Under the simplest breeding system and diversification model,  species are classified as self-incompatible $(I)$ or self-compatible $(C)$ (M11).
This is the same model as the analysis presented in \citet{goldberg_2010} but with an updated phylogeny \citep{sarkinen_2013} and larger aggregated dataset.
...
The simplest model classifies each species as either self-incompatible diploid $(ID)$, self-compatible diploid $(CD)$, or self-compatible polyploid $(CP)$; recall that self-incompatible polyploids do not occur (M16).
...
As for the previous models of only one trait, we then allow diversification to be influenced by only a hidden trait (M17), or by both ploidy and breeding system as well as a hidden trait (M18--M20, with varying degrees of complexity in the hidden trait transitions \cref{supptable:M16M20})
...
The models described so far allow us to assess the contributions of our two focal traits---ploidy and breeding system---to lineage diversification.
They do not, however, reveal whether it is `worth it' to include both traits in the analysis.
Model comparisons to test whether diversification heterogeneity is better explain by both traits than by either one require the calculation of the likelihood function of the data given an specific model.
However, the likelihood of the data given a model differs in the two-state versus the three-state models because the proposed models use different state spaces: $D/P$ versus $I/C$ versus $IC/CD/CP$. Therefore, different data results in incomparable models.

In order to compare model fits for ploidy-only versus breeding system-only versus both traits, we extend a new technique proposed in ontological work by \citet{tarasov_2019} that allows for state `lumping' .
...
We do not include additional models with diploidization because this reverse transition renders the models non-lumpable \citep{tarasov_2019}.
This means, that to lump two states a necessary and sufficient condition is that the rates going from the lumped state to the singular state are equal. 
...
% E: further explanation needed? R- I added a simple math explanation. Let me know if it is okay
...
Ancestral state estimations for all models show the maximum \emph{a posteriori} estimates of the marginal probability distributions for each of the 650 internal nodes for each of the models in \cref{figure:netdivall}. (\cref{suppfigure:DPnodipasr,suppfigure:DPnodipABasr,suppfigure:ICasr,suppfigure:ICABasr,suppfigure:IDCDCPnodipasr,suppfigure:IDCDCPnodipABasr}). % FIXME: I'm confused about if these are joint or marginal?  A bit more explanation would help. R: Marginal explanation added
...
\item Are hidden state necessary to add in this context?- Comparison of simple models against hidden state models (\cref{supptable:testaddhidden}).
\item Is a second trait adding information to the diversification process?- Comparison of lumped models against IC/CD/CP models (\cref{figure:lumped,table:lumped}).
...
\item Do asymmetrical rates in the model affect the pattern of diversification? - Comparison amongst hidden states models with equal hidden rates and asymmetrical rates (\cref{suppfigure:asymmetric,supptable:asymmetry}).
\end{enumerate}
...
\begin{figure}
\centering
\includegraphics[width=0.5\textwidth]{states.pdf} % FIXME: update numbers
\caption{
...
For example, there are 70 species that are SC with unknown ploidy. % Fixed
