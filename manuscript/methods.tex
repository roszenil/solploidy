...
%B: What happens if SC->SI is not fixed? Asking for a friend.
...
The ID/CD/CP model could potentially capture similar dynamics as earlier models, if the effects of the hidden state in D/P--A/B were effectively caused by breeding system (or its correlates), and the hidden state in I/C--A/B was effectively caused by ploidy. %B: you may wish to get rid of "(or its correlates)", which I inserted; along with "caused". I know that causation is not strict here, but it clarifies the intended biological explanation. %B: was: "if the hidden state in D/P--A/B was effectively breeding system, and the hidden state in I/C--A/B was effectively ploidy."
...
Parameters for each of the 10 diversification models were performed using custom code in the RevBayes \citep{hoehna_2016} environment. %B: fixme: I think the intent here is  "Parameter estimation [...] was peformed"
...
Using the marginal likelihood values, we calculated thirteen different Bayes factors. % E: it said "fourteen" but I only count thirteen? %B: fixme: my count too.
...
\end{tabular}
\caption{Marginal likelihoods for the 10 diversification models proposed.}
