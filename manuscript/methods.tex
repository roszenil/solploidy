...
The lumpability property requires that the rates going from the lumped state to the singular state are equal.
Under the diploidization models CP goes back to CD with rate $\delta$ but the rate from CP back to ID is 0, and $\delta \neq 0$.
Therefore, two-state models with diplodization cannot be re-coded to be compared to the tree-state models.
