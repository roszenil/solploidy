...
To address these questions, we employ an extended SSE model framework, which simultaneously models transitions between the discrete states of a trait and different rates of speciation and extinction associated with each of those states \citep{maddison_2007, fitzjohn_2012}.
...
First, we consider both ploidy and breeding system simultaneously, which reduces the chance that an effect of one trait on diversification would be mistaken for the effect of the other.
Second, we employ a hidden state model, which allows for an unknown factor to additionally influence diversification \citep{beaulieu_2016}.
...
Finally, for all models involving ploidy, we investigate the potential effects of including diploidization.
