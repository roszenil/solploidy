...
diploidsi<-which(matched$dat[,4]==2 & matched$dat[,5]=="SI")# 123
diploidsc<-which(matched$dat[,4]==2 & matched$dat[,5]=="SC")#160
diploidsna<-which(matched$dat[,4]==2 & is.na(matched$dat[,5])==TRUE) #178
polyploids<-which(matched$dat[,4]>2) #109
polyploidsc<-which(matched$dat[,4]>2 & matched$dat[,5]=="SC")#92
polyploidsna<-which(matched$dat[,4]>2 & is.na(matched$dat[,5])==TRUE)#17
si<-which(is.na(matched$dat[,4])==TRUE & matched$dat[,5]=="SI")#13
sc<-which(is.na(matched$dat[,4])==TRUE & matched$dat[,5]=="SC")#55
...
#Missing species from dataset
a_tmp<-matched$phy$tip.label
b_tmp<-dat2$Species
b_tmp %in% a_tmp 
dat2[639:651,]
#651 species!!!!
